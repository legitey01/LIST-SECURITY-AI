...
function buildMcpServers(
  ...
  const listSecurityAiHelperServer = createListSecurityAiHelperServer(sourceDir);
  ...
  if (agentName) {
    ...
    if (playwrightMcpName) {
      ...
      const isDocker = process.env.LIST_SECURITY_AI_DOCKER === 'true';
...
async function processMessageStream(
  ...
  for await (const message of query({ prompt: fullPrompt, options })) {
    ...
    const now = Date.now();
    if (global.LIST_SECURITY_AI_DISABLE_LOADER && now - lastHeartbeat > HEARTBEAT_INTERVAL) {
